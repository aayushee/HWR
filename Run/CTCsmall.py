...
def preprocess(stroke_dir, ascii_dir, data_file):
        ...
        #print (yarray)
        pickle.dump([xarray,yarray], f, protocol=2)
...
data_dir = "data"
data_file = os.path.join(data_dir, "strokes_training_data_small.cpkl")
...
x_train,y_train=load_preprocessed(data_file)
#print (y_train)
...
f2=open("stroks_padded_small.pkl","wb")
pickle.dump([x_train,y_train],f2,protocol=2)
f2.close()
...
rnn_encoded = Bidirectional(LSTM(128, return_sequences=True),
                                name='bidirectional_1',
                                merge_mode='concat',trainable=trainable)(inputs)
birnn_encoded = Bidirectional(LSTM(64, return_sequences=True),
...
output = TimeDistributed(Dense(66, name='dense'))(birnn_encoded)
...
#fit model with generator that generates batch samples 
model.fit_generator(my_generator,epochs=10,steps_per_epoch=20)

test_func = K.function([inputs], [y_pred])

letters=decode_batch(test_func,x_train[:5])
print (letters)
#fi=open('output10all.txt','w')
